...
- (void)showUsingAnimation:(BOOL)animated {
    if (animated && self.animationType == MBProgressHUDAnimationZoomIn) {
        self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
    } else if (animated && self.animationType == MBProgressHUDAnimationZoomOut) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (self.animationType == MBProgressHUDAnimationZoomIn || self.animationType == MBProgressHUDAnimationZoomOut) {
...
- (void)hideUsingAnimation:(BOOL)animated {
    ...
    if (animated && self.showStarted) {
        ...
        // in the done method
        if (self.animationType == MBProgressHUDAnimationZoomIn) {
            self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(1.5f, 1.5f));
        } else if (self.animationType == MBProgressHUDAnimationZoomOut) {
            self.transform = CGAffineTransformConcat(rotationTransform, CGAffineTransformMakeScale(0.5f, 0.5f));
        }
...
- (void)setTextColor:(UIColor *)aColor
{
    ...
    detailsLabel.textColor = _textColor;
    
    //    BOOL isActivityIndicator = [indicator isKindOfClass:[UIActivityIndicatorView class]];
    //    if (isActivityIndicator)
    //    {
    //        UIActivityIndicatorView * aIndicator = (UIActivityIndicatorView *)self.indicator;
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [self.indicator isKindOfClass:[MBRoundProgressView class]];
    
    if (self.mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
    ...
    else if (self.mode == MBProgressHUDModeDeterminate || self.mode == MBProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
        ...
        if (self.mode == MBProgressHUDModeAnnularDeterminate) {
            [(MBRoundProgressView *)self.indicator setAnnular:YES];
        }
    }
    else if (self.mode == MBProgressHUDModeCustomView && self.customView != self.indicator) {
...
- (void)layoutSubviews {
    ...
    CGFloat maxWidth = bounds.size.width - 2 * self.margin - 2 * self.outMargin;
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
    ...
    CGFloat remainingHeight = bounds.size.height - totalSize.height - kPadding - 2 * self.margin - 2 * self.outMargin;
...
#pragma clang diagnostic pop
    ...
    if (self.square) {
        ...
        if (max <= bounds.size.width - 2 * self.outMargin) {
            totalSize.width = max;
        }
        if (max <= bounds.size.height - 2 * self.outMargin) {
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.dimBackground) {
        ...
        CGFloat gradColors[8] = {0.0f,0.0f,0.0f,0.4f,0.0f,0.0f,0.0f,0.4f};
